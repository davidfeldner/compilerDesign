...
class SentinelLockQueue implements Queue {  
  ...
  private Node head = new Node(-444, null), tail = head;
  ...
  public synchronized int get2() {
    Node first = head.next;  //head peger stadig på S
    if (first == null) {
      return -999;
	  }
    
    head.next = first.next;   //opdaterer S.next til at pege på næste item    
    if (tail == first) {     //overholder invariant som specificeret ovenfor        
	    tail = head;
	  }
    return first.item;
  }
