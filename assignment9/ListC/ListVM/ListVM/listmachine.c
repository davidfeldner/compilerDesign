...
// Simple mark and sweep implementation from top of page 201 in PLC
// White = free    black = used     blue = on freelist (or too small / only header)
static void mark(word* block) {
  if (!inHeap(block)) return;
  word hdr = block[0];
  unsigned int col = Color(hdr);
  if (col == Blue || col == Black) return;
  block[0] = Paint(hdr, Black);
  /* Recursively mark any heap pointers in the block contents */
  unsigned int len = Length(hdr);
  for (unsigned int i = 1; i <= len; i++) {
    word v = block[i];
    if (!IsInt(v) && v != 0 && inHeap((word*)v))
      mark((word*)v);
  }
}
...
void markPhase(word s[], word sp) {
  printf("marking ...\n");
  /* Scan stack for references s[0..sp] */
  for (word i = 0; i <= sp; i++) {
    word v = s[i];
    if (v != 0 && !IsInt(v) && inHeap((word*)v)) {
      word* p = (word*)v;
      printf("Found heap reference at stack[" WORD_FMT "] to heap block at heap[" WORD_FMT "]\n",
             i, (word)(p - heap));
      mark(p);
    }
  }
...
/* Return total merged length (excluding header) for white blocks following heapPtr. */
unsigned int join_white(word* heapPtr) {
  word hdr = heapPtr[0];
  unsigned int total = Length(hdr);
  word* next = heapPtr + total + 1;
  while (next < afterHeap) {
    word nhdr = next[0];
    unsigned int nlen = Length(nhdr);
    unsigned int ncol = Color(nhdr);
    if (nlen > 0 && ncol == White) {
      total += nlen + 1;
      next = heapPtr + total + 1;
    } else break;
  }
  return total;
}
// after this, all blocks in the heap are white, except those on the freelist, which are blue (or too small which are also blue)
void sweepPhase() {
  printf("sweeping ...\n");
  word* heapPtr = heap;
  while (heapPtr < afterHeap) {
    ...
    if (len > 0) {
      if (col == White) {
        /* join_white returns the total length of merged words */
        unsigned int total = join_white(heapPtr);
        /* Make merged block blue and push onto freelist */
        word tag = BlockTag(hdr);
        heapPtr[0] = mkheader(tag, total, Blue);
        heapPtr[1] = (word)freelist;
        freelist = heapPtr;
        /* Advance past the merged block */
        heapPtr = heapPtr + total + 1;
      } else if (col == Black) {
        /* Live: reset color to white for next GC */
        heapPtr[0] = Paint(hdr, White);
        heapPtr = heapPtr + len + 1;
      } else {
        /* Blue: already on freelist, ignore */
        heapPtr = heapPtr + len + 1;
      }
    } else {
      /* Orphan (length 0): ignore */
      heapPtr += 1;
    }
  }
}
