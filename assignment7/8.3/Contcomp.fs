...
let allocate (kind: int -> var) (typ, x) (varEnv: varEnv) : varEnv * instr list =
    ...
    | TypA(t, Some i) ->
        let newEnv =
            ((x, (kind (fdepth + i), typ)) :: env, fdepth + i + 1) (* i+1 because we need room for the array elements and pointer to first element. *)
...
let rec cStmt stmt (varEnv: varEnv) (funEnv: funEnv) (C: instr list) : instr list =
    ...
    | While(e, body) ->
        ...
        let (jumptest, C1) = makeJump (cExpr e varEnv funEnv (IFNZRO labbegin :: C))
    ...
    | Block stmts ->
        ...
        pass2
            stmtsback
            (addINCSP (snd varEnv - fdepthend) C) (* Remove variables, declared in the block, from the stack *)
...
and cExpr (e: expr) (varEnv: varEnv) (funEnv: funEnv) (C: instr list) : instr list =
    ...
    | CstI i -> addCST i C
    | PreDec i -> cAccess i varEnv funEnv (DUP :: LDI :: CSTI -1 :: ADD :: STI :: C)
    | PreInc i -> cAccess i varEnv funEnv (DUP :: LDI :: CSTI 1 :: ADD :: STI :: C)
    ...
    | Andalso(e1, e2) ->
        match C with
        | IFZERO lab :: _ -> cExpr e1 varEnv funEnv (IFZERO lab :: cExpr e2 varEnv funEnv C)
        | IFNZRO labthen :: C1 ->
            let (labelse, C2) = addLabel C1
            cExpr e1 varEnv funEnv (IFZERO labelse :: cExpr e2 varEnv funEnv (IFNZRO labthen :: C2))
        | _ ->
            let (jumpend, C1) = makeJump C
    ...
    | Orelse(e1, e2) ->
        match C with
        | IFNZRO lab :: _ -> cExpr e1 varEnv funEnv (IFNZRO lab :: cExpr e2 varEnv funEnv C)
        | IFZERO labthen :: C1 ->
            let (labelse, C2) = addLabel C1
            cExpr e1 varEnv funEnv (IFNZRO labelse :: cExpr e2 varEnv funEnv (IFZERO labthen :: C2))
        | _ ->
            let (jumpend, C1) = makeJump C
            let (labtrue, C2) = addLabel (addCST 1 C1)
            cExpr e1 varEnv funEnv (IFNZRO labtrue :: cExpr e2 varEnv funEnv (addJump jumpend C2))
