...
let rec addCST i C =
    ...
    | (x, CSTI y :: SWAP :: LT ::  NOT :: C1) -> if x >= y then addCST 1 C1 else addCST 0 C1
    | (x, CSTI y :: SWAP :: LT ::  C1) -> if x > y then  addCST 1 C1 else addCST 0 C1
    | (x, CSTI y :: LT :: NOT :: C1) -> if x <= y then  addCST 1 C1 else addCST 0 C1
    | (x, CSTI y :: LT :: C1) -> if x < y then  addCST 1 C1 else addCST 0 C1
    | (x, CSTI y :: EQ :: C1) -> if x = y then  addCST 1 C1 else addCST 0 C1
    | (x, CSTI y :: NOT :: EQ :: C1) -> if x <> y then  addCST 1 C1 else addCST 0 C1
    | _                     -> CSTI i :: C
...
let addIFZERO lab3 C =
  match C with
  | (GOTO l1 :: Label l2 :: C1) when l2 = lab3 -> IFNZRO l1 :: Label lab3 :: C1
  | _ -> IFZERO lab3 :: C
...
let rec cStmt stmt (varEnv : varEnv) (funEnv : funEnv) (C : instr list) : instr list = 
    ...
    | While(e, body) ->
      ...
      let (jumptest, C1) = 
           makeJump (cExpr e varEnv funEnv (addIFZERO labbegin C))
...
and cExpr (e : expr) (varEnv : varEnv) (funEnv : funEnv) (C : instr list) : instr list =
    ...
    | Ternary(eCond, eTrue, eFalse) ->
        let (jumpend, C1) = makeJump C
        let (labelse, C2) = addLabel (cExpr eFalse varEnv funEnv C1)
        cExpr eCond varEnv funEnv (addIFZERO labelse (cExpr eTrue varEnv funEnv (addJump jumpend C2)))
