...
let rec ppValue = function
  ...
  | List l -> let elems = List.map ppValue l
              "[" + (String.concat "," elems) + "]"
  | PairV (e1, e2) -> "(" + ppValue e1 + ", " + ppValue e2 + ")"
  | ClosureRef closRef ->
    match !closRef with
...
let rec evalExpr (env : value env) (e : expr<typ>)
  ...
  | Prim1(ope,e1,_) ->
    evalExpr env e1 (fun v1 ->
                     ...
                     | ("fst",_) -> match v1 with //NEW 
                                    | PairV(v1, _) -> cont v1
                                    | _ -> Abort ("Prim1: fst on non-pair value: " + (ppValue v1))
                     | ("snd",_) -> match v1 with
                                    | PairV(_, v2) -> cont v2
                                    | _ -> Abort ("Prim1: snd on non-pair value: " + (ppValue v1))
                     | _ -> Abort ("Prim1 "+ope+" not implemented")) econt
  | Pair(e1, e2, _) ->
    evalExpr env e1
      (fun v1 ->
       evalExpr env e2
         (fun v2 -> cont (PairV(v1, v2))) econt) econt
  | Prim2(ope, e1, e2,_) ->
    evalExpr env e1
      (fun v1 ->
       evalExpr env e2
...
let check e =
  let rec check' (e : expr<'a>) (env : string list) : bool =
    ...
    | Pair(v1, v2, _) -> check' v1 env && check' v2 env
