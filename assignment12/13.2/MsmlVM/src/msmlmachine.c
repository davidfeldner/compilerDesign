...
void printInstruction(word p[], word pc) {
  ...
  case PAIR:   printf("PAIR " WORD_FMT " " WORD_FMT, p[pc+1], p[pc+2]);
  case PRINTP:  printf("PRINTP " WORD_FMT, p[pc+1]); break;
...
void printP(word i) {
    ...
    word fst = Untag(consPtr[1]);
    word snd = Untag(consPtr[2]);
    printf("(" WORD_FMT ", " WORD_FMT ")", fst, snd);
  return;
}
