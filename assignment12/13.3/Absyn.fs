...
let alphaConv p : program<'a> =
  ...
  let rec aExpr env e =
    ...
    | CstB (b, a) -> CstB (b, a)
    | CstN a -> CstN a
    | Var (v, a) -> Var (lookup v env, a)
    | Prim1(op, e1, a) -> Prim1(op, aExpr env e1, a)
    | Prim2(op, e1, e2, a) -> Prim2(op, aExpr env e1, aExpr env e2, a)
    | AndAlso(e1, e2, a) -> AndAlso(aExpr env e1, aExpr env e2, a)
    | OrElse(e1, e2, a) -> OrElse(aExpr env e1, aExpr env e2, a)
    | If(e1, e2, e3) -> If(aExpr env e1, aExpr env e2, aExpr env e3)
    | Let(valdecs, body) -> 
        let env', valdecs' = aValdecs env valdecs
        Let(valdecs', aExpr env' body)
    | Seq(e1, e2, a) -> Seq(aExpr env e1, aExpr env e2, a)
    | Fun(x, body, a) -> 
        let x' = freshVar x
        let env' = (x, x') :: env
        Fun(x', aExpr env' body, a)
    | Call(e1, e2, tOpt, a) -> Call(aExpr env e1, aExpr env e2, tOpt, a)
    | Raise(e, a) -> Raise(aExpr env e, a)
    | TryWith(e1, ExnVar exn, e2) ->
        let exn' = freshVar exn
        let env' = (exn, exn') :: env
        TryWith(aExpr env e1, ExnVar exn', aExpr env' e2)
  
  and aValdec' env = function
    | Valdec(x, eRhs) -> 
        let x' = freshVar x
        let env' = (x, x') :: env
        env', Valdec(x', aExpr env eRhs)
    | Exn(ExnVar exn, a) ->
        let exn' = freshVar exn
        let env' = (exn, exn') :: env
        env', Exn(ExnVar exn', a)
    | Fundecs fs ->
        let fNames = List.map (fun (f, _, _) -> f) fs
        let fNames' = List.map freshVar fNames
        let fEnv = List.zip fNames fNames'
        let env' = fEnv @ env
        let fs' = List.map (fun (f, x, body) ->
                      let f' = lookup f env'
                      let x' = freshVar x
                      let env'' = (x, x') :: env'
                      f', x', aExpr env'' body) fs
        env', Fundecs fs'
  
  and aValdecs env valdecs =
    match valdecs with
    | [] -> env, []
    | vd :: vds ->
        let env', vd' = aValdec' env vd
        let env'', vds' = aValdecs env' vds
        env'', vd' :: vds'
  
  and aProg' = function
    | Prog(valdecs, body) -> 
        let env', valdecs' = aValdecs [] valdecs
        Prog(valdecs', aExpr env' body)
